...
from collective.taskqueue2.views.taskstatus import ITaskstatus
...
class ViewsIntegrationTest(unittest.TestCase):
    ...
    def test_taskstatus_is_registered(self):
        view = getMultiAdapter(
            (self.portal['other-folder'], self.portal.REQUEST),
            name='taskstatus'
        )
        self.assertTrue(ITaskstatus.providedBy(view))

    def test_taskstatus_not_matching_interface(self):
        view_found = True
        try:
            view = getMultiAdapter(
            ...
            view_found = ITaskstatus.providedBy(view)
